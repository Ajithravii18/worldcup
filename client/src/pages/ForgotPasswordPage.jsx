    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-4" style={{ background: '#f8fafc' }}>
      ...
      <div className="w-full max-w-sm animate-slide-up relative z-10">
        <div className="relative p-8 overflow-hidden" style={{ background: '#ffffff', border: '1px solid #e2e8f0' }}>
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            {['email', 'otp', 'done'].map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className="w-2 h-2 transition-all duration-300" style={{ background: ['email','otp','done'].indexOf(step) >= i ? '#F26522' : '#e2e8f0' }} />
                {i < 2 && <div className="h-px w-8 transition-all" style={{ background: ['email','otp','done'].indexOf(step) > i ? '#F26522' : '#e2e8f0' }} />}
              </div>
            ))}
          </div>
          ...
          {step === 'email' && (
            <>
              <h2 className="font-display text-lg font-black tracking-[0.2em] mb-2 text-center uppercase" style={{ color: '#0f172a' }}>Forgot Password</h2>
              <p className="text-center text-[11px] tracking-widest uppercase mb-7 font-bold" style={{ color: '#64748b' }}>Enter your email and a new password</p>
              ...
              <form onSubmit={handleSendOtp} className={`space-y-5 transition-opacity ${loading ? 'opacity-50' : ''}`}>
                <div className="relative pt-4">
                  ...
                  <label className="absolute left-0 top-4 text-xs tracking-[0.2em] uppercase font-bold transition-all duration-300 peer-focus:top-0 peer-focus:text-[10px] peer-valid:top-0 peer-valid:text-[10px]" style={{ color: '#64748b' }} htmlFor="forgot-email">Email</label>
                </div>
                ...
                <div className="relative pt-4">
                  <input id="forgot-new-password" type={showPassword ? 'text' : 'password'} value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
                    ...
                    onFocus={e => e.currentTarget.style.borderBottomColor = '#F26522'}
                    onBlur={e => e.currentTarget.style.borderBottomColor = '#e2e8f0'} />
                  <label className="absolute left-0 top-4 text-xs tracking-[0.2em] uppercase font-bold transition-all duration-300 peer-focus:top-0 peer-focus:text-[10px] peer-valid:top-0 peer-valid:text-[10px]" style={{ color: '#64748b' }} htmlFor="forgot-new-password">New Password</label>
          ...
          {step === 'otp' && (
            <>
              <h2 className="font-display text-lg font-black tracking-[0.2em] mb-2 text-center uppercase" style={{ color: '#0f172a' }}>Verify Code</h2>
                ...
                <div className="flex justify-center gap-3 mb-7" onPaste={handleOtpPaste}>
                  {otp.map((digit, i) => (
                    <input key={i} ref={otpRefs[i]} id={`forgot-otp-${i}`}
                      ...
                      style={{ borderBottom: `2px solid ${digit ? '#F26522' : '#e2e8f0'}`, color: '#0f172a' }}
                    />
                  ))}
                </div>
